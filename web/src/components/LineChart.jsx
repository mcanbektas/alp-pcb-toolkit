...
// Örnekleme kuralı: her `every` adımda bir indeks, son nokta her zaman dahil
// (eğrinin sağ ucu asimptotu taşır, atlanmamalı). Saf fonksiyon — bileşenin
// kendisi dışında da kullanılabilir (örn. report.js'in grafik veri tablosu),
// böylece iki yer aynı satırları farklı kurallarla seçip sessizce ayrışmaz.
export function sampleIndices(length, every) {
  const indices = []
...
export function ChartDataTable({ xLabel, series, formatX, formatY, every = 1 }) {
  ...
  const ref = live[0].points
  const indices = sampleIndices(ref.length, every)
