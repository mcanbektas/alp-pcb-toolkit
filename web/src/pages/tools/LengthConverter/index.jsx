...
export default function LengthConverter() {
  ...
  const saved = useSavedCalculation({
    toolKey: 'length-conv', initialForm: INITIAL_FORM, patch,
  })
  const { lang } = useLang()
  ...
  // En yalın pilot: şematik (cetvel) ve grafik kapsam dışı, yalnızca birim
  // tablosu raporlanır (bkz. plan §13).
  const reportSection = useMemo(() => buildReportSection({ f, r, text }), [f, r, text])
  ...
  return (
    <>
      ...
      <ReportDialog section={reportSection} />
      <SaveToProject toolKey="length-conv" f={f} r={r} section={reportSection} saved={saved} />
    </>
  )
}
