...
import {
  ...
  TOOL_OHM, TOOL_COMBO, COMBO_PARALLEL, COMBO_SERIES,
} from './model'
...
describe('OhmLaw report.js', () => {
  ...
  it('Ohm kanunu — üç değer birden girilince tutarsızlık satırı eklenir', () => {
    const { section } = buildFor(TOOL_OHM, { I: '30', Iu: 'mA' })
  ...
  it('Ohm kanunu — yetersiz girdide compute() başarısız olur, null döner (ekranla aynı kural)', () => {
    ...
    const r = compute(TOOL_OHM, f, text.fieldLabels)
  ...
  it('Seri/paralel birleşim — direnç sayısı kadar pay satırı döner, grafik yoktur', () => {
    const { section } = buildFor(TOOL_COMBO, { combo: COMBO_PARALLEL, values: '10k 22k 47k' })
  ...
  it('Seri/paralel birleşim (seri) — pay satırı başlığı text.table.shareSeries olur', () => {
    const { section } = buildFor(TOOL_COMBO, { combo: COMBO_SERIES, values: '10k 22k 47k' })
  ...
  it('Seri/paralel birleşim — okunamayan değer listesinde null döner', () => {
    ...
    const r = compute(TOOL_COMBO, f, text.fieldLabels)
    expect(r.ok).toBe(false)
    expect(buildReportSection({ f, r, text })).toBeNull()
  })

  it('sweep (s) verilince Ohm için chart.table dolu döner, svg alanı boş bırakılır', () => {
    const { section } = buildFor(TOOL_OHM)
  ...
  it('chart tablosu taramanın son satırını hiç atlamaz (ekrandaki kuralla aynı)', () => {
    const { s, section } = buildFor(TOOL_OHM)
  ...
  it('hiçbir sonuç satırı, tüm dallarda, boş etiket ya da undefined değer taşımaz', () => {
    const cases = [
      buildFor(TOOL_OHM),
      buildFor(TOOL_OHM, { I: '30', Iu: 'mA' }),
      buildFor(TOOL_COMBO, { combo: COMBO_PARALLEL, values: '10k 22k 47k' }),
    ]
  ...
  it('schematicCaption — ekrandaki <figcaption> ile aynı metni taşır (ohm/combo)', () => {
    expect(buildFor(TOOL_OHM).section.schematicCaption).toBe(text.schematic.caption[TOOL_OHM])

    const { section: seriesSection } = buildFor(
      TOOL_COMBO, { combo: COMBO_SERIES, values: '10k 22k 47k' },
    )
    expect(seriesSection.schematicCaption).toBe(text.schematic.captionSeries)

    const { section: parallelSection } = buildFor(
      TOOL_COMBO, { combo: COMBO_PARALLEL, values: '10k 22k 47k' },
    )
  ...
  it('İngilizce metinle çağrıldığında araç adı ve mod da İngilizce', () => {
    const en = getText('en')
    const f = { ...INITIAL_FORM, tool: TOOL_OHM }
    const r = compute(TOOL_OHM, f, en.fieldLabels)
