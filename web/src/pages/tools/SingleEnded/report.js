...
// `fs` alan çözücünün BİTMİŞ ve hatasız sonucudur (yoksa null) — F2:
// çözücü satırları rapora da girer. İndirme anında çözüm sürüyorsa satırlar
// rapora girmez; grounded CPW'de (solverOnly) rapor o durumda sayı içermez,
// "hesaplanıyor" değeriyle dürüst kalır.
export function buildReportSection({ mode, f, r, s, text, fs }) {
  if (!r.ok) return null
  ...
  const big = r.mode === MODE_SYNTHESIS
    ...
    : {
      ...
      ...(r.solverOnly
        ? (fs ? splitFormatted(fmtRes(fs.Z0, 4)) : { value: text.bigResultPending })
        : splitFormatted(fmtRes(r.Z0, 4))),
      emphasis: true,
    }
  ...
  const results = [
    ...
    ...(r.solverOnly
      ? [
        ...(fs
          ? [
            { label: text.table.z0, ...splitFormatted(fmtRes(fs.Z0, 5)) },
            { label: text.table.epsEff, value: fmt(fs.epsEff, 5) },
        ...
        ...(fs
          ? [
            ...
            { label: text.table.model, value: fs.model },
          ]
          : []),
      ]
      : [
        ...
        ...(fs
          ? [
            {
              label: r.mode === MODE_SYNTHESIS ? text.solver.rowZ0Syn : text.solver.rowZ0,
              ...splitFormatted(fmtRes(fs.Z0, 5)),
            },
            { label: text.solver.rowEps, value: fmt(fs.epsEff, 5) },
            { label: text.solver.rowConv, value: text.pct(fmt(fs.convergence.coarsePct, 2)) },
            { label: text.solver.rowMethod, value: fs.method },
          ]
          : []),
      ]),
  ]
